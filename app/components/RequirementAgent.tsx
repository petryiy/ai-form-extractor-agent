...
const welcomeMessage: Message = {
  ...
  role: "assistant",
  content:
    "你好，我会先了解你们的业务背景、当前问题和项目约束。你可以从想解决的业务问题开始讲。"
};
...
function renderValue(value: RequirementState[RequirementKey]) {
...
export function RequirementAgent({ showInspector = false }: RequirementAgentProps) {
  ...
  }, [hydrated, messages, requirementState, sessionId]);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`${json?.error ?? "需求抽取失败，请稍后重试。"}${details}`);
      }
      ...
      <section className="workspace">
        <header className="topbar">
          <div>
            ...
            <h1>{showInspector ? "AI 需求获取代理" : "需求交流"}</h1>
          </div>
          <div className="topbar-actions">
            <button className="icon-button" type="button" onClick={handleReset} title="重置会话">
              <RotateCcw size={18} />
            </button>
            {isLoading ? (
        ...
        <div className={showInspector ? "main-grid" : "customer-grid"}>
          <section className="chat-panel" aria-label="客户对话">
            <div className="panel-header">
              <div>
                <h2>{showInspector ? "客户对话" : "在线沟通"}</h2>
                <p>{isBusy ? "正在处理最新回复" : "等待客户输入"}</p>
              </div>
              {showInspector ? (
            ...
            <aside className="insight-panel" aria-label="结构化需求表">
                ...
                <div>
                  ...
                  <p>
                    {missingCriteria.length > 0
                      ? `${missingCriteria.length} 个必填项待确认`
                      : "必填项已齐全"}
                  </p>
                </div>
                  ...
                  return (
                    <div className="criterion" key={criterion.key}>
                      <div className="criterion-title">
                        ...
                        {criterion.required ? <strong>必填</strong> : null}
                      </div>
                      <div className="criterion-value">{renderValue(value)}</div>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="json-panel">
                <div className="json-header">
                  <h3>JSON 输出</h3>
                  <button className="icon-button" type="button" onClick={copyJson} title="复制 JSON">
                    <Copy size={16} />
                  </button>
                </div>
